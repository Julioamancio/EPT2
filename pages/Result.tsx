    ...
    <div className="max-w-4xl mx-auto px-4 py-16 min-h-screen">
      <div className="bg-white border border-slate-100 rounded-[3.5rem] overflow-hidden shadow-[0_30px_100px_-20px_rgba(0,0,0,0.1)] print:shadow-none print:border-none print:rounded-none">
        ...
        <div className={`p-16 text-center print:hidden ${isApproved ? (finalLevel === ProficiencyLevel.C1 ? 'bg-amber-50/40' : 'bg-indigo-50/40') : 'bg-red-50/40'}`}>
          ...
          <h1 className="text-5xl font-black text-slate-900 mb-4 tracking-tight">Assessment Completed</h1>
          <p className="text-xl text-slate-600 font-medium max-w-xl mx-auto leading-relaxed">
            {isApproved 
              ? `Congratulations! You have been certified at international level ${finalLevel}.`
              : `Result: Your final score was ${score}%, below the minimum for certification.`}
          </p>
        </div>
        ...
        <div className="p-16 print:p-0">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16 print:hidden">
            <div className="p-10 bg-white border border-slate-100 rounded-[2.5rem] shadow-sm flex flex-col items-center text-center">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3">Final Achievement</p>
            ...
            <div className="p-10 bg-white border border-slate-100 rounded-[2.5rem] shadow-sm flex flex-col items-center text-center">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3">Qualification Obtained</p>
              <p className={`text-2xl font-black uppercase tracking-widest ${isApproved ? 'text-slate-900' : 'text-slate-300'}`}>
                {isApproved ? `Level ${finalLevel}` : 'Not Qualified'}
              </p>
              {isApproved && (
                <span className="text-[10px] font-bold text-slate-400 mt-2 uppercase flex items-center gap-2"><ShieldCheck className="w-3 h-3 text-emerald-500" /> Audited Record</span>
              )}
            </div>
          </div>
          ...
          {isApproved && certificate ? (
            <div className="animate-in fade-in slide-in-from-bottom-6 duration-700">
              {/* DYNAMIC CERTIFICATE AREA WITH IMPORTED BACKGROUND */}
              <div className="relative group max-w-2xl mx-auto mb-16 print:max-w-none print:m-0 print:block overflow-hidden rounded-[2rem] print:rounded-none">
                ...
                <div className="relative w-full aspect-[1.414/1] bg-white shadow-2xl print:shadow-none overflow-hidden border border-slate-200">
                  {certTemplate ? (
                    ...
                    <div className="w-full h-full bg-slate-50 flex items-center justify-center border-2 border-dashed">Waiting for Template...</div>
                  )}
                  
                  {/* DYNAMIC DATA OVERLAY - Centrally positioned to fit Oxford model */}
                  <div className="absolute inset-0 flex flex-col items-center justify-center p-[8%] text-center">
                    <div className="mt-[2%] space-y-[2vw] w-full">
                       ...
                       <div className="space-y-[1vw]">
                         <p className="text-[1.2vw] font-medium text-slate-500 print:text-lg">has successfully completed the proficiency exam level</p>
                       ...
                       <div className="pt-[4vw] flex justify-center gap-[15%]">
                         <div className="text-center">
                            <p className="text-[0.7vw] font-black text-slate-400 uppercase tracking-widest print:text-[10px]">Date of Issue</p>
                            <p className="text-[1.2vw] font-bold text-slate-700 print:text-lg">{certificate.date}</p>
                         </div>
                         <div className="text-center">
                            <p className="text-[0.7vw] font-black text-slate-400 uppercase tracking-widest print:text-[10px]">Authenticity (Hash)</p>
                 ...
                 <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-10">
                   <div>
                     <h3 className="text-2xl font-black flex items-center gap-3">
                       <GraduationCap className="text-indigo-400 w-6 h-6" /> Document Ready
                     </h3>
                     <p className="text-slate-400 font-medium mt-2 max-w-sm">You can print now or save as PDF for your resume.</p>
                   </div>
                   <div className="flex flex-col gap-4 w-full md:w-auto">
                      ...
                      <div className="px-6 py-4 border border-slate-700 rounded-2xl text-[10px] font-bold text-slate-500 uppercase flex items-center justify-center gap-3 tracking-widest">
                        VALID RECORD: {certificate.uniqueCode}
                      </div>
                   </div>
                 </div>
              </div>
            ...
            <div className="bg-red-50 border border-red-100 rounded-[2.5rem] p-12 text-center print:hidden">
              ...
              <h3 className="text-xl font-bold text-red-900 mb-2">Attention Candidate</h3>
              <p className="text-red-700 text-sm leading-relaxed max-w-md mx-auto font-medium">
                Unfortunately, the score obtained is not sufficient for the issuance of the B2/C1 proficiency certification. We recommend further study and a new attempt in 30 days.
              </p>
            </div>
          )}
          ...
          <div className="mt-16 text-center print:hidden">
             <Link to="/" className="inline-flex items-center gap-2 text-sm font-black text-slate-400 hover:text-indigo-600 uppercase tracking-widest transition-all group">
               Return to Main Portal <ChevronRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
             </Link>
          </div>
        </div>
      </div>
